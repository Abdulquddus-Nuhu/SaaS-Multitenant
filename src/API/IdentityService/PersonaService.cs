...
namespace Core.Services
{
    public class PersonaService : IPersonaService
    {
        ...
        public async Task<ApiResponse<ParentResponse>> CreateParentAsync(CreateParentRequest request, string host)
        {
            ...
            var user = new Persona() { Id = Guid.NewGuid().ToString(), UserName = request.Email, Email = request.Email, PhoneNumber = request.PhoneNumber, FirstName = request.FirstName, LastName = request.LastName, EmailConfirmed = true, PhotoUrl = photoUrl, PesonaType = PersonaType.Parent };
        ...
        public async Task<ApiResponse<StudentResponse>> CreateStudentAsync(CreateStudentRequest request, string host)
        {
            ...
            if (sameEmailCount is not 0)
            {
                ...
                user = new Persona() { Id = Guid.NewGuid().ToString(), FirstName = request.FirstName, LastName = request.LastName, Email = emailExist, UserName = string.Concat(request.FirstName, request.LastName), PhotoUrl = photoUrl, PesonaType = PersonaType.Student, EmailConfirmed = true };
            }
            else
            {
                user = new Persona() { Id = Guid.NewGuid().ToString(), FirstName = request.FirstName, LastName = request.LastName, Email = string.Concat(request.FirstName, request.LastName, "@", "smsabuja", ".com"), UserName = string.Concat(request.FirstName,  request.LastName), PhotoUrl = photoUrl, PesonaType = PersonaType.Student, EmailConfirmed = true };
            }
        ...
        public async Task<BaseResponse> DeleteUserByAdminAsync(Guid userId)
        {
            ...
            var persona = await _dbContext.Users.FirstOrDefaultAsync(x => x.Id == userId.ToString());
            if (persona is null)
            {
