...
namespace API.MultitenantSetup
{
    public class TenantChangeService : ITenantChangeService
    {
        ...
        public TenantChangeService(AppDbContext context, ILogger<TenantChangeService> logger)
        {
            _context = context;
            _logger = logger;
        }
        ...
        public async Task<string> SingleTenantDeleteAsync(Tenant tenant)
        {
            ...
            try
            {
                ...
                var deleteSalesSql = $"DELETE FROM invoice.{nameof(AppDbContext.Grades)} WHERE DataKey = '{dataKey}'";
                await _context.Database.ExecuteSqlRawAsync(deleteSalesSql);
                var deleteStockSql = $"DELETE FROM invoice.{nameof(AppDbContext.JobTitles)} WHERE DataKey = '{dataKey}'";
