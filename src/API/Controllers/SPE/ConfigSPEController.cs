...
namespace API.Controllers.SPE
{
        ...
        IDepartmentService departmentService) : BaseController
    {
        ...
        {
            var datakey = User.Claims.FirstOrDefault(x => x.Type == "datakey");
            var response = await _campusService.CreateCampus(request, User.Identity!.Name ?? string.Empty, datakey.Value ?? string.Empty);
